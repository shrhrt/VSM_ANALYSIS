...
class VSMApp:
    ...
    def _create_menu(self):
        ...
        file_menu.add_command(
            label="ツール: PPMSデータ(.dat)を変換...",
            command=self._show_conversion_tool,
        )
    ...
    def _show_conversion_tool(self):
        """datファイルをvsmファイルに変換するツールを呼び出す"""
        try:
            # toolsフォルダからモジュールとして直接読み込む
            from tools import dat_to_VSM

            messagebox.showinfo(
                "ステップ1",
                "変換したい.datファイルを選択してください。",
                parent=self.root,
            )
            ...
            dat_to_VSM.convert_dat_to_vsm(dat_path, vsm_path)
            messagebox.showinfo(
                "成功",
                f"変換が完了しました。\nファイルは以下に保存されました:\n{vsm_path}",
                parent=self.root,
