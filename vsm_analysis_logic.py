...
def format_axis(ax, fig, style_params, unit_mode="SI (T, kA/m)"):
    ...
    # 単位モードに応じて軸ラベルを設定
    if "CGS" in unit_mode:
        ax.set_xlabel(r"$H$ ($\mathrm{Oe}$)", fontsize=style_params.get("axis_label_fontsize", 16))
        ax.set_ylabel(r"$M$ ($\mathrm{emu/cm^3}$)", fontsize=style_params.get("axis_label_fontsize", 16))
    elif "Normalized" in unit_mode:
        ax.set_xlabel(r"$\mu_0H$ ($\mathrm{T}$)", fontsize=style_params.get("axis_label_fontsize", 16))
        ax.set_ylabel(r"$M/M_s$", fontsize=style_params.get("axis_label_fontsize", 16))
    else:  # SI (T, kA/m)
        ax.set_xlabel(r"$\mu_0H$ ($\mathrm{T}$)", fontsize=style_params.get("axis_label_fontsize", 16))
        ax.set_ylabel(r"$M$ ($\mathrm{kA/m}$)", fontsize=style_params.get("axis_label_fontsize", 16))
