...
class VSMApp:
    ...
    def _create_analysis_controls(self, parent):
        ...
        self.thick_entry = ttk.Entry(settings_frame, textvariable=self.thick_var, width=10)
        self.thick_entry.grid(row=1, column=1, sticky="ew", padx=5)
        ttk.Label(settings_frame, text="基板面積 (cm²):").grid(row=2, column=0, sticky="w")
        ...
        self.offset_check = ttk.Checkbutton(settings_frame, text="磁化オフセット補正", variable=self.offset_correction_var)
        self.offset_check.grid(row=3, column=0, columnspan=2, sticky="w", pady=(5, 0))
        self.legend_check = ttk.Checkbutton(settings_frame, text="凡例を表示", variable=self.show_legend_var)
        ...
        self.demag_check = ttk.Checkbutton(demag_frame, text="反磁場補正を有効化", variable=self.demag_correction_var)
        self.demag_check.grid(row=0, column=0, columnspan=4, sticky="w")
        self.manual_check = ttk.Checkbutton(demag_frame, text="傾き計算の範囲を手動指定", variable=self.manual_slope_var)
        ...
        self.pos_h_min_entry = ttk.Entry(demag_frame, textvariable=self.pos_h_min_var, width=7)
        self.pos_h_min_entry.grid(row=2, column=1, sticky="ew")
        ttk.Label(demag_frame, text="～").grid(row=2, column=2)
        self.pos_h_max_entry = ttk.Entry(demag_frame, textvariable=self.pos_h_max_var, width=7)
        self.pos_h_max_entry.grid(row=2, column=3, sticky="ew")
        ttk.Label(demag_frame, text="負 H (T):").grid(row=3, column=0, sticky="w", pady=(5, 0))
        self.neg_h_min_entry = ttk.Entry(demag_frame, textvariable=self.neg_h_min_var, width=7)
        self.neg_h_min_entry.grid(row=3, column=1, sticky="ew", pady=(5, 0))
        ttk.Label(demag_frame, text="～").grid(row=3, column=2, pady=(5, 0))
        self.neg_h_max_entry = ttk.Entry(demag_frame, textvariable=self.neg_h_max_var, width=7)
    ...
    def _add_traces(self):
        trace_vars = [self.thick_var, self.area_var, self.offset_correction_var, self.show_legend_var, self.marker_size_var,
                      ...
                      self.demag_correction_var, self.manual_slope_var, self.pos_h_min_var, self.pos_h_max_var, self.neg_h_min_var, self.neg_h_max_var,
                      self.unit_mode_var]
        for var in trace_vars: var.trace_add("write", self._schedule_update)
        self.demag_correction_var.trace_add("write", lambda *a: self.toggle_demag_fields())
    ...
    def _create_individual_color_widget(self, index, filename, color_var):
        ...
        ttk.Label(row_frame, text=f"{display_name}:").pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(0, 5))
    ...
    def toggle_demag_fields(self):
        ...
        for widget in [self.pos_h_min_entry, self.pos_h_max_entry, self.neg_h_min_entry, self.neg_h_max_entry]:
            ...
            try:
                ...
                if self.demag_correction_var.get():
                    ...
                    if self.manual_slope_var.get():
                        print("    傾き計算: 手動設定モード")
                        try:
                            pos_range = (float(self.pos_h_min_var.get()), float(self.pos_h_max_var.get()))
                            neg_range = (float(self.neg_h_min_var.get()), float(self.neg_h_max_var.get()))
                            slope, r2_pos, r2_neg = vsm_logic.find_demag_slope_manual(H_raw, M_raw, pos_range, neg_range)
                        except ValueError:
                            print("  エラー: 手動設定の磁場範囲が無効。"); slope, r2_pos, r2_neg = 0, 0, 0
                    else:
                        print("    傾き計算: 自動検出モード")
                        slope, r2_pos, r2_neg = vsm_logic.find_demag_slope_auto(H_raw, M_raw)
                    print(f"    補正傾き S: {slope:.6f}, R^2: [正 {r2_pos:.4f}], [負 {r2_neg:.4f}]")
                else:
                    print("  反磁場補正: 無効"); slope = 0
                ...
                Ms_avg = vsm_logic.calculate_saturation_magnetization(H_raw, M_final)
                ...
                self.ax.plot(H_plot_up, M_plot_up, color=color, label=file.stem, **plot_kwargs)
