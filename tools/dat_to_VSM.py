...
def convert_dat_to_vsm(dat_file_path: str, vsm_file_path: str) -> None:
    """
    ...
    Raises:
        IOError: .datファイルの読み込みに失敗した場合。
        ValueError: .datファイルにデータが見つからなかった場合、または必須列が見つからなかった場合。
    """
    # --- ステップ1: .datファイルの解析 ---
    header_info: Dict[str, List[str]] = {}
    data_lines: List[List[str]] = []
    is_data_section: bool = False
    ...
    sample_name = header_info.get("TITLE", [""])[0].strip() or os.path.basename(
        dat_file_path
    ).replace(".dat", "")
    ...
    measuring_time_min = (
        (time_stamps.iloc[-1] - time_stamps.iloc[0]) / 60 if meas_points > 1 else 0
    )
    ...
    with open(vsm_file_path, "w", encoding="utf-8") as f:
        ...
        for index, row in df.iterrows():
            current_time = start_datetime + timedelta(
                seconds=(row["Time Stamp (sec)"] - time_offset)
            )
...
def main() -> None:
    ...
    default_vsm_name = (
        os.path.basename(dat_path).replace(".dat", ".vsm").replace(".DAT", ".vsm")
    )
    messagebox.showinfo(
        "ステップ2", "変換後の.VSMファイルの保存場所と名前を指定してください。"
    )
