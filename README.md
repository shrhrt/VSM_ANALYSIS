...
![データ読み込み](assets/Gif/diamagnetic_correction.gif)
...
- **モダンなUI/UX**: `sv-ttk` テーマを採用し、Windows 11にネイティブに馴染むデザインと、ダークモード / ライトモードの動的切り替えを提供。

### 4. データ管理と出力

![解析結果](assets/images/result.png)

- **セッションの保存・復元**: 現在読み込んでいるファイルパスのリストや、各ファイルに対する個別の補正設定（膜厚、色、補正範囲など）を `.vsm_session` ファイルとして保存できる。相対パスによる復元アルゴリズムを実装しており、別のPCやクラウドストレージ上でも作業を完全に再現可能。
...
- **実行ログの記録**: 解析の過程やエラー情報、保存時の詳細などを「ログ」タブに記録し、トラブルシューティングを容易。
...
## 今後の展望
本アプリケーションは現在も継続的に改善を行っており、今後は以下を予定。

- **総合的な材料解析プラットフォームへの拡張 (汎用性の拡大)**: 現在のVSM（M-Hカーブ）解析に加え、X線回折（XRD）や磁化の温度依存性（M-Tカーブ）など、異なる測定手法のデータも管理・比較できるソフトウェアへと進化。
- **パフォーマンスチューニングと起動の高速化**: アプリケーションの起動時間を短縮するため、重量級ライブラリ（PandasやMatplotlib等）の遅延読み込みや非同期初期化を検討。
- **直感的なUI/UXの追求**: グラフ上でのマウスドラッグによる補正範囲のインタラクティブな指定や、パラメータ変更時のリアルタイムプレビューなど、シームレスな操作性をさらに強化。
- **CI/CDパイプラインの構築と自動運用**: GitHub Actionsを導入し、テスト（`pytest`）の自動実行、静的コード解析、および PyInstaller を用いた各OS向け実行可能ファイル（`.exe`）の自動ビルド・リリース体制を構築し、モダンな開発・デプロイフローを実現。